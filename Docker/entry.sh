...
# 下载程序
function download(){
  [ -d /tmp/bili_BiliExper-clannad ] && rm -rf /tmp/bili_BiliExper-clannad
  wget -q -O /tmp/BiliExp.zip https://archive.fastgit.org/FairyWorld/bili_BiliExper/archive/clannad.zip
  [ "$?" != 0 ] && echo "Update/Download BiliExp error!" && return
  unzip /tmp/BiliExp.zip -d /tmp > /dev/null
  rm /tmp/BiliExp.zip
  mv /tmp/bili_BiliExper-clannad* /tmp/BiliExp
